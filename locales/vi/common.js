module.exports = {
  'switch_language': 'Tiếng Việt',

  'contact_us': 'Liên hệ',
  'contact_name': 'Tên',
  'contact_gender': 'Giới tính',
  'contact_email': 'Email',
  'contact_message': 'Nhập nội dung tin nhắn...',
  'contact_send': 'Gửi',
  'contact_sending': 'Đang gửi...',

  'step_basic_info': 'Thông tin cơ bản',
  'step_description_vietnamese': 'Mô tả bằng tiếng Việt',
  'step_description_english': 'Mô tả bằng tiếng Anh',
  'step_select_thumbnail': 'Đăng ảnh bìa',
  'step_config_carousel_images': 'Upload gallery images',
  'step_address_location': 'Địa chỉ',
  'step_done': 'Hoàn thành đăng tin',

  'ok': 'Bước tiếp theo',

  'title_in_vietnamese': 'Tiêu đề bằng tiếng Việt',
  'title_in_english': 'Tiêu đề bằng tiếng Anh',
  'price': 'Giá',
  'price_in_vnd': 'Giá VND',
  'price_in_usd': 'Giá USD',
  'category': 'Danh mục',
  'sale_type': 'Sales type',
  'rental_period': 'Rental period',
  'rental_period_value': 'Number',
  'rental_period_unit': 'Period',
  'available_until': 'Available until',
  'facing_direction': 'Hướng',
  'bed_room_count': 'Số lượng phòng ngủ',
  'size': 'Diện tích',
  'size_width': 'Chiều rộng',
  'size_length': 'Chiều dài',

  'buy': 'Mua',
  'rent': 'Cho thuê',
  'north': 'Bắc',
  'north-east': 'Đông Bắc',
  'east': 'Đông',
  'south-east': 'Đông Nam',
  'south': 'Nam',
  'south-west': 'Tây Nam',
  'west': 'Tây',
  'north-west': 'Tây Bắc',
  'days': 'Day(s)',
  'months': 'Month(s)',
  'quarters': 'Quarter(s)',
  'years': 'Year(s)',

  'upload_image': 'Đăng ảnh',

  'address_in_vietnamese': 'Địa chỉ bằng tiếng Việt',
  'address_in_english': 'Địa chỉ bằng tiếng Anh',
  'address_visible': 'Hiển thị địa chỉ cho người dùng',

  'create_the_property': 'Đăng tin',
  'create_the_property_desc': 'The property will be created by data from all the previous steps',

  'update_the_property': 'Cập nhật tin',
  'update_the_property_desc': 'The property will be updated by data from all previous steps',

  'property_delete_confirm': 'ABạn có chắc muốn xóa tin này?',

  'contact_request_form_validate_warn_title': 'Thông tin nhập không đúng',
  'contact_request_form_validate_warn_body': 'Nhập địa chỉ email và nội dung tin nhắn!',

  'contact_request_success_title': 'Tin nhắn đã được gửi đi thành công',
  'contact_request_success_body': 'Chúng tôi sẽ sớm phản hồi tin nhắn của bạn!',

  'contact_request_error_title': 'Có lỗi xảy ra khi gửi tin nhắn',
  'contact_request_error_body': 'Bấm lại nút gửi hoặc tải lại trang!',

  'menu_home': 'Trang chủ',
  'menu_about_us': 'Về chúng tôi',
  'menu_properties_list': 'Bất động sản',
  'menu_contact_us': 'Liên hệ',

  'switch_currency_label': 'Tiền tệ: ',

  'footer_phone_info': `
    <strong>English speaker:</strong>
    <p>Dean Walkerden (+84) 981 688 075</p>
    <strong>French-English-Vietnamese speaker:</strong>
    <p>Sonia-Phuong Tran (+84) 981 688 076</p>
  `,
  'footer_copyright': 'Copy &copy; 2016 by YOUR SIDE',

  'home_slogan': 'Một cuộc sống mới bắt đầu',
  'search_bar_heading': 'Tìm kiếm',
  'home_lastest_properties': 'Bất động sản mới nhất',
  'home_message': '"Chúng tôi lựa chon trang web này dựa trên uy tín về chất lượng cũng như dịch vụ khách hàng tốt"',
  'home_message_btn': 'Thêm thông tin',

  'search_by_name': 'Nhập địa điểm',
  'search_all_categories': 'Tất cả',
  'search_min_bed': 'Số phòng ngủ tối thiểu',
  'search_max_bed': 'Số phòng ngủ tối đa',
  'search_min_price': 'Giá thấp nhất',
  'search_max_price': 'Giá cao nhất',
  'search_district': 'Quận/Huyện',
  'search_size': 'Diện tích',
  'search_btn': 'Tìm kiếm',

  'district-1': 'Quận 1',
  'district-2': 'Quận 2',
  'district-3': 'Quận 3',
  'district-4': 'Quận 4',
  'district-5': 'Quận 5',
  'district-6': 'Quận 6',
  'district-7': 'Quận 7',
  'district-8': 'Quận 8',
  'district-9': 'Quận 9',
  'district-10': 'Quận 10',
  'district-11': 'Quận 11',
  'district-12': 'Quận 12',
  'binh-thanh': 'Quận Bình Thạnh',
  'tan-binh': 'Quận Tân Bình',
  'phu-nhuan': 'Quận Phú Nhuận',
  'tan-phu': 'Quận Tân Phú',
  'go-vap': 'Quận Gò Vấp',
  'binh-tan': 'Quận Bình Tân',
  'thu-duc': 'Quận Thủ Đức',
  'binh-chanh': 'Huyện Bình Chánh',
  'nha-be': 'Huyện Nhà Bè',
  'hoc-mon': 'Huyện Hóc Môn',
  'cu-chi': 'Huyện Củ Chi',
  'can-gio': 'Huyện Cần Giờ',


  'size_lte_30': '<= 30 m2',
  'size_30-50': '30 - 50 m2',
  'size_50-80': '50 - 80 m2',
  'size_80-100': '50 - 100 m2',
  'size_100-150': '100 - 150 m2',
  'size_150-200': '150 - 200 m2',
  'size_200-250': '200 - 250 m2',
  'size_250-300': '250 - 300 m2',
  'size_300-500': '300 - 500 m2',
  'size_gte_500': '>= 500 m2',

  'about_us_heading': 'Giới thiệu',
  'about_us_content': `
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean in eleifend ipsum. Duis hendrerit turpis et sapien hendrerit, et convallis ligula ultrices. Integer venenatis venenatis neque non feugiat. Maecenas pretium nisi a pharetra malesuada. Sed bibendum lorem eu elit mattis pellentesque. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Sed fermentum dapibus nibh, eget commodo ante porttitor ut. Suspendisse pulvinar, magna id pulvinar mattis, tellus purus faucibus lorem, non porttitor neque massa quis odio. Duis lobortis suscipit nunc, id consectetur nisl vestibulum at. Curabitur id dui lacinia, porttitor ex vitae, sollicitudin lectus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Praesent vel metus porttitor elit tincidunt rutrum eget a leo. Nulla facilisi. Praesent ut sollicitudin mauris. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas.</p>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean in eleifend ipsum. Duis hendrerit turpis et sapien hendrerit, et convallis ligula ultrices. Integer venenatis venenatis neque non feugiat. Maecenas pretium nisi a pharetra malesuada. Sed bibendum lorem eu elit mattis pellentesque. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Sed fermentum dapibus nibh, eget commodo ante porttitor ut. Suspendisse pulvinar, magna id pulvinar mattis, tellus purus faucibus lorem, non porttitor neque massa quis odio. Duis lobortis suscipit nunc, id consectetur nisl vestibulum at. Curabitur id dui lacinia, porttitor ex vitae, sollicitudin lectus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Praesent vel metus porttitor elit tincidunt rutrum eget a leo. Nulla facilisi. Praesent ut sollicitudin mauris. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas.</p>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean in eleifend ipsum. Duis hendrerit turpis et sapien hendrerit, et convallis ligula ultrices. Integer venenatis venenatis neque non feugiat. Maecenas pretium nisi a pharetra malesuada. Sed bibendum lorem eu elit mattis pellentesque. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Sed fermentum dapibus nibh, eget commodo ante porttitor ut. Suspendisse pulvinar, magna id pulvinar mattis, tellus purus faucibus lorem, non porttitor neque massa quis odio. Duis lobortis suscipit nunc, id consectetur nisl vestibulum at. Curabitur id dui lacinia, porttitor ex vitae, sollicitudin lectus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Praesent vel metus porttitor elit tincidunt rutrum eget a leo. Nulla facilisi. Praesent ut sollicitudin mauris. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas.</p>
  `,

  'unknown': 'NA',

  'detail_size': 'Diện tích',
  'detail_beds': 'Phòng ngủ',
  'district': 'Quận',
  'detail_price': 'Giá',
  'detail_available_until': 'Available',
  'detail_facing_direction': 'Hướng',

  'login': 'Đăng nhập',
  'username': 'Tên đăng nhập',
  'password': 'Mật khẩu',
  'login_error': 'Kiểm tra lại tên đăng nhập và mật khẩu!',

  'delete': 'Xóa',
  'edit': 'Sửa',
  'link': 'Liên kết',
  'property_name': 'Property Name',
  'created_date': 'Ngày tạo',
  'commands': 'Commands',
  'view_page': 'View in page',

  'previous': 'Quay lại',
  'next': 'Bước tiếp theo',

  'close_contact_form': 'Đóng',
  'property_location': 'Vị trí'
}
